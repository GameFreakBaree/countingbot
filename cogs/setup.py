...
class Setup(commands.Cog):
    ...
    async def autosetup(self, ctx):
        ...
        if result_tuple[1] == 0:
            ...
            counting_cursor.execute(
                f"UPDATE counting_guildsettings SET counting_channel_id = {counting_channel.id} WHERE guild_id = {guild}")
            db_counting.commit()
    ...
    async def autosetup_error(self, ctx, error):
        ...
        if isinstance(error, discord.Forbidden):
            await ctx.send("The bot has not enough permissions. "
                           ...
                           "Then you can remove those permissions.\n"
                           "If you don't want that, just make sure the bot has the permission **Manage Permissions** in the counting channel.\n"
                           "After that, run the ` c!setup ` command in that channel.")
